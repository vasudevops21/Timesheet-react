...
const TimeoffSetup = ({ locale, setLocale }) => {
  const [isAddTimeOffModalOpen, setIsAddTimeOffModalOpen] = useState(false);
  const [isEditTimeOffModalOpen, setIsEditTimeOffModalOpen] = useState(false);
  const [timeOffs, setTimeOffs] = useState([]);
  const [editingTimeOffIndex, setEditingTimeOffIndex] = useState(null);
  const [timeOffToDeleteIndex, setTimeOffToDeleteIndex] = useState(null);
  ...
  const fetchTimeOffs = async () => {
    try {
      console.log('Fetching time offs...');
      const response = await axios.get(process.env.REACT_APP_API_BASE_URL + "/api/v1/TimeoffService/getAllTimeoff");
      setTimeOffs(response.data);
    } catch (error) {
      console.error('Error fetching time offs:', error);
      toast.error('Failed to fetch time offs');
    }
  };
  ...
  const openEditTimeOffModal = (index) => {
    setIsAddTimeOffModalOpen(false);
    setIsEditTimeOffModalOpen(true);
    setEditingTimeOffIndex(index);
  };

  const handleSaveTimeOff = async (editedTimeOff) => {
    try {
      if (editingTimeOffIndex !== null) {
        // Update existing time off
        await axios.post(process.env.REACT_APP_API_BASE_URL + "/api/v1/TimeoffService/update", editedTimeOff);
        const updatedTimeOffs = [...timeOffs];
        updatedTimeOffs[editingTimeOffIndex] = editedTimeOff;
        setTimeOffs(updatedTimeOffs);
        toast.success('Time off updated successfully');
      } else {
        // Add new time off
        const response = await axios.post(process.env.REACT_APP_API_BASE_URL + "/api/v1/TimeoffService/save", editedTimeOff);
        setTimeOffs([...timeOffs, { ...editedTimeOff, Timeoffid: response.data }]);
  ...
  // const deleteTimeOff = async (index) => {
  //   try {
  //     await axios.delete( `http://192.168.1.203:8080/api/v1/TimeoffService/delete/${timeOffs[index].Timeoffid}`);
  //     const updatedTimeOffs = [...timeOffs];
  //     updatedTimeOffs.splice(index, 1);
  //     setTimeOffs(updatedTimeOffs);
  ...
  const deleteTimeOff = async (index) => {
    try {
      const timeoffId = timeOffs[index].timeoffId; // Ensure the correct property is used
      await axios.delete(`http://192.168.1.203:8080/api/v1/TimeoffService/delete/${timeoffId}`);

      // Filter out the deleted item from the timeOffs array
      const updatedTimeOffs = timeOffs.filter((_, i) => i !== index);
      setTimeOffs(updatedTimeOffs);
  ...
  return (
    <div>
      ...
      <div className="d-flex">
        ...
        <div className='Timeoff_fullbody'>
          <SettingTopBar />
          <div className="">
            <TimeOff
              isVisible={isAddTimeOffModalOpen || isEditTimeOffModalOpen}
              onClose={closeTimeOffModal}
              onSave={handleSaveTimeOff}
              initialTimeOff={editingTimeOffIndex !== null ? timeOffs[editingTimeOffIndex] : null}
              isEditing={editingTimeOffIndex !== null}
            />
            <div className='p-5 mx-5 settings-Timeoff-body-h'>
              ...
              <div className='settings-Timeoff-body'>
                <table className="table settings-Timeoff-table">
                  <thead className='settigs-Timeoff-table-head'>
                    <tr className='settings-Timeoff-border-none'>
                      <th scope="col" className='settings-Timeoff-heading1 no-border '>
                        <div className='settings-Timeoff-first-heading heading-div'>Time Off Name</div>
                      </th>
                      <th scope="col" className='settings-Timeoff-heading2 no-border'>
                        <div className='settings-Timeoff-mid-heading heading-div'>Time Off charge code</div>
                      </th>
                      <th scope="col" className='settings-Timeoff-heading4 no-border'>
                        <div className='settings-Timeoff-last-heading heading-div'>Pay Type</div>
                      </th>
                      <th scope="col" className='settings-Timeoff-heading3 no-border'>
                        <div className='settings-Timeoff-last-heading heading-div'>Action</div>
                      </th>
                    </tr>
                  </thead>
                  <tbody className='setting_Timeoff_table_row'>
                    {timeOffs.map((timeOff, index) => (
                      <tr className="setting_Timeoff_table_row_item" key={index}>
                        <td className="setting_Timeoff_table_cell">
                          <div className='settings-Timeoff-first-heading'>{timeOff.timeoffname}</div>
                        </td>
                        <td className="setting_Timeoff_table_cell">
                          <div className='settings-Timeoff-mid-heading'>{timeOff.timeoffchargecode}</div>
                        </td>
                        <td className="setting_Timeoff_table_cell">
                          <div className='settings-Timeoff-last-heading'>{timeOff.paytype}</div>
                        </td>

                        <td className="setting_Timeoff_table_cell">
                          <div className="settings-Timeoff-action-button">
                            <button className="btn btn-sm btn-primary me-2 setting_Timeoff_edit_btn" onClick={() => openEditTimeOffModal(index)}>
                            ...
                            <button
                              ...
                              onClick={() => setTimeOffToDeleteIndex(index)}
                              data-bs-toggle="modal"
                              data-bs-target="#conformationdeleteTimeoffModal"
                            >
            ...
            <div className="modal setting_popup_box_Timeoff_blur" id="conformationdeleteTimeoffModal" tabIndex="-1" aria-labelledby="conformationdeleteTimeoffModalLabel" aria-hidden="true">
              <div className="modal-dialog setting_popup_box_Timeoff_modal_dialog">
                <div className="modal-content setting_popup_box_Timeoff_modal_contect">
                  <div className="modal-body" id="conformationdeleteTimeoffModalLabel">
                    <div className="setting_role_delete_Timeoff_confirm_close" data-bs-dismiss="modal" aria-label="Close">
                      <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="-3 0 22 18" fill="none">
                        <path d="M4 12L12 4M12 12L4 4" stroke="#F8F8F8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    </div>
                    <div className='d-flex mt-3'>
                      <div className='setting_role_delete_svg_Timeoff'>
                      ...
                      <div className='confirmdelete_word_Timeoff'>
                        <h2 className='confirmdelete_word_h2_Timeoff'>Delete Time Off</h2>
                        <p className='confirmdelete_word_p_Timeoff'>Are you certain you wish to proceed with the deletion of this?</p>
                      </div>
                    </div>
                  </div>
                  <div className='d-flex justify-content-center mb-5'>
                    <div
                      className="button btn d-flex flex-row setting_role_delete_button_Timeoff"
                      onClick={() => {
                        if (timeOffToDeleteIndex !== null) {
                          deleteTimeOff(timeOffToDeleteIndex);
                          setTimeOffToDeleteIndex(null);
                        }
                      }}
                    ...
                    >
                      ...
                      <div className="setting_role_delete_button_text_Timeoff">Delete</div>
                    </div>
                    <div className="button btn d-flex flex-row setting_role_cancel_button_Timeoff" data-bs-dismiss="modal">
                      ...
                      <div className="setting_role_cancel_button_text_Timeoff">Cancel</div>
                    </div>
                  </div>
                </div>
              </div>
...
export default TimeoffSetup;

const TimeOff = ({ isVisible, onClose, onSave, initialTimeOff, isEditing }) => {
  const [timeOffName, setTimeOffName] = useState('');
  const [timeOffChargeCode, setTimeOffChargeCode] = useState('');
  const [payType, setPayType] = useState('paid');

  useEffect(() => {
    if (initialTimeOff) {
      setTimeOffName(initialTimeOff.timeoffname);
      setTimeOffChargeCode(initialTimeOff.timeoffchargecode);
      setPayType(initialTimeOff.paytype || 'paid');
    } else {
      setTimeOffName('');
      setTimeOffChargeCode('');
    }
  }, [initialTimeOff]);

  const handleSaveClick = () => {
    if (timeOffName.trim() && timeOffChargeCode.trim()) {
      const timeOffData = {
        Timeoffid: initialTimeOff ? initialTimeOff.Timeoffid : null,
        timeoffname: timeOffName,
        timeoffchargecode: timeOffChargeCode,
        paytype: payType
      };
      onSave(timeOffData);
      setTimeOffName('');
      ...
      toast('Please Enter Time Off Name & Charge Code');
    }
  };
  ...
  return (
    <div className="Settings-Timeoff-popup-modal">
      <div className="Settings-Timeoff-popup-content">
        <div className='Settings-Timeoff-popup-setupTimeoff'>
          <p className='Settings-Timeoff-popup-headline'>{isEditing ? 'Edit Time Off' : 'Setup Time Off'}</p>
          <div className='Settings-Timeoff-popup_cancel_button' onClick={handleClose}>
            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
              <path d="M6 18L18 6M18 18L6 6" stroke="#F8F8F8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
        </div>
        <div className='Settings-Timeoff-popup-fill-text'>
          <p className="Settings-Timeoff-popup-instruction-text">
            {isEditing ? 'PLEASE UPDATE THE DETAILS' : 'PLEASE FILL THE DETAILS'}
          </p>
        </div>
        <div className='Settings-Timeoff-popup-Timeoff'>
          <div className='Settings-Timeoff-popup-add-Timeoff'>
            <label className='Settings-Timeoff-popup-add-Timeoff-input'>Time Off Name</label>
            <input
              className='Settings-Timeoff-popup-add-Timeoff-input-placeholder'
              type="text"
              value={timeOffName}
          ...
          <div className='Settings-Timeoff-popup-add-date'>
            <label className='Settings-Timeoff-popup-add-date-input'>Charge Code</label>
            <input
              className='Settings-Timeoff-popup-add-Timeoff-input-placeholder'
              type="text"
              value={timeOffChargeCode}
              onChange={(e) => setTimeOffChargeCode(e.target.value)}
              placeholder="Charge Code"
              required
            />
          </div>
        </div>
        <div className='Settings-Timeoff-popup-add-pay-type'>
          <label className='Settings-Timeoff-popup-add-pay-type-input'>Pay Type</label>
        ...
        <div className="Settings-Timeoff-popup-button-group d-flex gap-4 justify-content-center ms-4">
          <div className='d-flex'>
            {isEditing ? (
              <button className='Settings-Timeoff-popup-button-group-edit_button btn px-3' onClick={handleSaveClick}>
              ...
              <button className='Settings-Timeoff-popup-button-group-add_button btn btn-sm px-4 py-2' onClick={handleSaveClick}>
          ...
          <div className='settings-button-cancel'>
            <button className='Settings-Timeoff-popup-edit-cancel-button btn px-3' onClick={handleClose}>
